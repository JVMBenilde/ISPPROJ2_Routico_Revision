...
class NotificationService {
  constructor(db) {
    ...
    const vonageApiSecret = process.env.VONAGE_API_SECRET || null;
    this.vonageFrom = process.env.VONAGE_FROM || 'Routico';

    if (vonageApiKey && vonageApiSecret) {
      this.vonageClient = new Vonage({ apiKey: vonageApiKey, apiSecret: vonageApiSecret });
    } else {
  ...
  normalizePhone(phone) {
    ...
    // SMSAPI PH commonly expects E.164 format.
  ...
  async sendSms(userIds, smsMessage) {
    ...
    for (const user of users) {
      ...
      try {
        ...
        if (msg.status !== '0') {
          failed += 1;
          console.error(`SMS send failed for user ${user.user_id}: ${msg['error-text'] || `status ${msg.status}`}`);
          continue;
        }
        ...
        console.error(`SMS send failed for user ${user.user_id}: ${details}`);
      }
    }
  ...
  async notifyUsers(userIds, payload) {
    ...
    console.log(
      `[Notify:${notificationType}] push(sent=${pushResult.sent || 0}, failed=${pushResult.failed || 0}, skipped=${pushResult.skipped ? 'yes' : 'no'}${pushResult.reason ? `, reason=${pushResult.reason}` : ''}) ` +
      `sms(sent=${smsResult.sent || 0}, failed=${smsResult.failed || 0}, skipped=${smsResult.skipped ? 'yes' : 'no'}${smsResult.reason ? `, reason=${smsResult.reason}` : ''})`
    );
  }
